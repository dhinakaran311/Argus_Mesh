...
export default async function RingDetailPage({ params }: Props) {
  ...
  try { cluster = await api.cluster(clusterId) } catch {}
  try { flowData = await api.graph(clusterId) } catch {}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem', fontSize: '0.8rem' }}>
              {([
                ...
                <div key={label} style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid #C8C2B266', paddingBottom: '0.45rem' }}>
                  ...
                  <span style={{ fontFamily: mono ? 'JetBrains Mono, monospace' : 'Inter', fontWeight: 500, color: 'var(--ink-paper)', fontSize: mono ? '0.8rem' : '0.8rem' }}>
                    {String(value)}
                  </span>
                </div>
              ))}
        ...
        <div style={{ height: 580, border: '1px solid var(--rule)', overflow: 'hidden' }}>
          {flowData && flowData.nodes.length > 0 ? (
            ...
            <div className="corkboard" style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: '0.5rem' }}>
              ...
              <div style={{ color: 'var(--muted)', fontSize: '0.72rem' }}>Neo4j connection required</div>
            </div>
          )}
        </div>
      </div>
